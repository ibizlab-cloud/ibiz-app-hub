...
import { isWeekday, baseUnit, getMillisecondBy } from '@/utils/date';
...
class GanttHeader extends Header {
  ...
  minLength: number = 0;
  showWeekdays: number[] = [0, 1, 2, 3, 4, 5, 6];
  ...
  setDate(
    ...
    this.minLength = minLen;
    if (showWeekdays && showWeekdays.length > 0) {
  ...
  generate() {
    ...
    for (s = start; s <= end; ) {
      const d = new XDate(s);
      d.startOf(this.unit);
      if (isWeekday(d as any, this.showWeekdays)) {
        this.dates.push(d);
      }
