...
export class XDate {
  ...
  constructor(date?: LikeDate) {
    // 如果date为空字符串，day计算出的时间为无效时间 赋值未定义后会拿默认当天的时间
    this.date = day(date || undefined).toDate();
  }
   ...
   */
  isValid(date?: XDate): boolean {
    return date ? day(date.date).isValid() : day(this.date).isValid();
  }
}
