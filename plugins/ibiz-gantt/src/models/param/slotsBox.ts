...
export default class SlotsBox {
  ...
  ganttCell!: VNode | null;

  ganttTitle!: VNode | null;
  ...
  setSlots(slots: Slots | VNode[]) {
    ...
    if (!Array.isArray(slots)) {
      if (slots.ganttCell) {
        this.ganttCell = slots.ganttCell as unknown as VNode;
      }
      if (slots.ganttTitle) {
        this.ganttTitle = slots.ganttTitle as unknown as VNode;
      }
      if (slots.empty) {
        this.empty = slots.empty as unknown as VNode;
      }
      if (slots.setting) {
        this.setting = slots.setting as unknown as VNode;
      }
    }
  }
}
