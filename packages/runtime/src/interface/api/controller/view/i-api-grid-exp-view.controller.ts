...
import { IApiGridExpViewState } from '../../state';
import { IApiMDViewCall } from '../../call';
import { IApiViewController } from './i-api-view.controller';

/**
 * @description 实体表格导航视图控制器接口
 * @export
 * @interface IApiGridExpViewController
 * @extends {IApiViewController<IAppDEGridExplorerView, IApiGridExpViewState>}
 */
export interface IApiGridExpViewController
  extends IApiViewController<IAppDEGridExplorerView, IApiGridExpViewState> {
  /**
   * @description 执行视图的能力
   * @template T
   ...
   *         : []} args 视图能力需要的参数
   * @returns {*}  {Promise<any>}
   * @memberof IApiGridExpViewController
   */
