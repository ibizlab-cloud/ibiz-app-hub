...
import { IApiEditView2State } from '../../state';
import { IApiEditViewCall } from '../../call';
import { IApiViewController } from './i-api-view.controller';

/**
 * @description 实体编辑视图（左右关系）控制器接口
 * @export
 * @interface IApiEditView2Controller
 * @extends {IApiViewController<IAppDEEditView, IApiEditView2State>}
 */
export interface IApiEditView2Controller
  extends IApiViewController<IAppDEEditView, IApiEditView2State> {
  /**
   * @description 执行视图的能力
   * @template T
   ...
   *         : []} args 视图能力需要的参数
   * @returns {*}  {Promise<any>}
   * @memberof IApiEditView2Controller
   */
